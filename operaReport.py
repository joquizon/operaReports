...
from pdfcreator import coverCreate
from pdfcreator import report_merger
...
def createFolder():
    def folderError():
        ...
        lbl1 = Label(window, text=f'ERROR!!!\n{Targfold[0]}\nalready Exists!!!\nMove folder then\nClick Try Again',
                     bg=labelBG, fg='black', font='helvetica 14 italic')
        lbl1.grid(row=0, column='0', padx=0, pady=5, sticky=N)

        tryagain = Button(window, text=f'TRY\nAgain', font='helvetica 12 italic', width=9, height=4, background='red',
                          fg='white', command=TryAgain).grid(row=8, column=0, padx=0, pady=5, sticky=N)
    ...
    def folderMessage():
        ...
        lbl1 = Label(window, text=f'Download\nxml and pdf files\ninto folder\n{Targfold[0]}', bg=labelBG, fg='black',
                     font='helvetica 14 italic')
        lbl1.grid(row=0, column='0', padx=0, pady=5, sticky=N)

        start = Button(window, text=f'Files\nReady', font='helvetica 12 italic', width=9, height=4,
                       background='dodger blue', fg='white', command=start).grid(row=8, column=0, padx=0, pady=5,
                                                                                 sticky=N)
...
def fileStarter():
    ...
    def fileMiss():
        ...
        Label(window, text=f'{len(MissXmlsList)} issues ...{MissXmlsList}', bg=labelBG, fg='violet red',
              font='helvetica 12').grid(row=1, column='0', padx=20, pady=0, sticky=W)
        Label(window, text=f'{len(DupXmlsList)} issues ...{DupXmlsList}', bg=labelBG, fg='violet red',
              font='helvetica 12').grid(row=2, column='0', padx=20, pady=0, sticky=W)
        Label(window, text=f'{len(MissPdflist)} issues ...{MissPdflist}', bg=labelBG, fg='purple3',
              font='helvetica 12').grid(row=3, column='0', padx=20, pady=0, sticky=W)
        Label(window, text=f'{len(DupPdflist)} issues ...{DupPdflist}', bg=labelBG, fg='purple3',
              font='helvetica 12').grid(row=4, column='0', padx=20, pady=0, sticky=W)
        ...
        lbl1 = Label(window, text=f'!!!!!!!!\nFILE MISSING IN\n{Targfold[0]}\nMissing contents:', bg=labelBG,
                     fg='black', font='helvetica 14 italic')
        lbl1.grid(row=0, column='0', padx=20, pady=5, sticky=W)

        start = Button(window, text=f'Files\nReady', font='helvetica 12 italic', width=9, height=4,
                       background='dodger blue', command=start).grid(row=30, column=0, padx=20, pady=20, sticky=W)
    ...
    for file in range(len(filesContained)):
        if filesContained[file].startswith('man') and filesContained[file].endswith('xml'):
            ...
            wtflist.append(filesContained[file])
...
linefind = mgrf.findall(
    'LIST_G_MASTER_VALUE_ORDER/G_MASTER_VALUE_ORDER/LIST_G_LAST_YEAR_01/G_LAST_YEAR_01/LIST_G_CROSS/G_CROSS/LIST_G_MASTER_VALUE/G_MASTER_VALUE/SUB_GRP_1_ORDER')

for lf in range(0, 99):
    if linefind[lf].text == '30000':
        mroute = mgrf.findall(
            f'LIST_G_MASTER_VALUE_ORDER/G_MASTER_VALUE_ORDER/LIST_G_LAST_YEAR_01/G_LAST_YEAR_01/LIST_G_CROSS/G_CROSS/LIST_G_MASTER_VALUE/G_MASTER_VALUE[{lf}]/LIST_G_HEADING_1_ORDER/G_HEADING_1_ORDER/LIST_G_SUM_AMOUNT/G_SUM_AMOUNT/SUM_AMOUNT')
        #         total rooms
...
detavTitle = [[f'DETAILED AVAILABILITY as of {audited}']]

# detailed av table >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
detailedavail = [['QBS'], ['SJR'], ['VIP'], ['TOTAL']]
...
detailedavail[0].append(qbs)
detailedavail[1].append(numeros[1])
detailedavail[2].append(vip)
detailedavail[3].append(sum(numeros))
...
rtefind = taxf.findall(
    'LIST_G_GROUP_BY_STATUS/G_GROUP_BY_STATUS/LIST_G_NAME_TAX_DESCRIPTION/G_NAME_TAX_DESCRIPTION/LIST_G_RESV_NAME_ID/G_RESV_NAME_ID')
...
def groupEntry():
    ...
    lbl1 = Label(window, text=F'please enter group name for\n{ArrName[0].text}\n or hit <enter> for none: ', bg=labelBG, fg=labelFG,
                 font=labelfont)
...
coverCreate(directory,title,entryName,stattitle,statistics,detavTitle,detailedavail,taxExmpt,zeroRateRooms,walkData,LateCXlData,arrivalMain,mainForecastList)

report_merger(directory)